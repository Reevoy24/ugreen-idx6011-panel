...
[![Release](https://img.shields.io/badge/release-v1.7.6-2ea44f)](../../releases/latest)
...
Installing **ug-paneld** gives you the touch dashboard **and** fan monitoring and control in one step, because fan control is bundled. The front LEDs are a separate, optional setup (see [Front panel LEDs](#front-panel-leds)). The commands below install **v1.7.6**. If a newer release exists, swap that version into the URL and filename (see the [releases page](../../releases)).

> [!NOTE]
> **Which build do I need?**
> * **iDX6011 Pro** (it has the front display): use the **panel** packages below.
> * **non-Pro iDX6011** (it has no display): skip the panel and install the display-free [**ug-fand v1.1.2**](https://github.com/Reevoy24/ugreen-idx6011-panel/releases/tag/ug-fand-v1.1.2) build instead (`.deb` for Proxmox, tarballs for TrueNAS/Unraid). See [Fan control](#fan-control).

### Proxmox / Debian

```bash
wget https://github.com/Reevoy24/ugreen-idx6011-panel/releases/download/v1.7.6/ug-paneld_1.7.6_amd64.deb
dpkg -i ug-paneld_1.7.6_amd64.deb
```

This installs the binary and enables and starts the service. A no-blacklist variant (`ug-paneld_1.7.6_no-blacklist_amd64.deb`) is available if you would rather manage the `i2c-hid-acpi` module yourself.
...
```bash
wget https://github.com/Reevoy24/ugreen-idx6011-panel/releases/download/v1.7.6/ug-paneld_1.7.6_truenas_amd64.tar.gz
tar xzf ug-paneld_1.7.6_truenas_amd64.tar.gz && cd ug-paneld
...
```bash
wget https://github.com/Reevoy24/ugreen-idx6011-panel/releases/download/v1.7.6/ug-paneld_1.7.6_unraid_amd64.tar.gz
tar xzf ug-paneld_1.7.6_unraid_amd64.tar.gz && cd ug-paneld
sh install.sh
```
...
> [!NOTE]
> **Non-Pro iDX6011?** The panel packages in [Install](#install) are for the Pro and drive the display. The non-Pro iDX6011 has no panel, so install the display-free [**ug-fand v1.1.2**](https://github.com/Reevoy24/ugreen-idx6011-panel/releases/tag/ug-fand-v1.1.2) build instead (a `.deb` for Proxmox, tarballs for TrueNAS/Unraid, config persists across reboots and OS updates). Everything below applies to it too.
...
**Standalone (non-Pro, no panel):** download the display-free [ug-fand v1.1.2](https://github.com/Reevoy24/ugreen-idx6011-panel/releases/tag/ug-fand-v1.1.2) build and install it. The commands install v1.1.2; swap in a newer version if one exists.

Proxmox / Debian:

```bash
wget https://github.com/Reevoy24/ugreen-idx6011-panel/releases/download/ug-fand-v1.1.2/ug-fand_1.1.2_amd64.deb
dpkg -i ug-fand_1.1.2_amd64.deb
```

TrueNAS SCALE:

```bash
wget https://github.com/Reevoy24/ugreen-idx6011-panel/releases/download/ug-fand-v1.1.2/ug-fand_1.1.2_truenas_amd64.tar.gz
tar xzf ug-fand_1.1.2_truenas_amd64.tar.gz && cd ug-fand
...
```bash
wget https://github.com/Reevoy24/ugreen-idx6011-panel/releases/download/ug-fand-v1.1.2/ug-fand_1.1.2_unraid_amd64.tar.gz
tar xzf ug-fand_1.1.2_unraid_amd64.tar.gz && cd ug-fand
sh install.sh
```
